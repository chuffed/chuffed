...
class BoundedPathPropagator : public GraphPropagator {
	...
	public:
		...
		virtual bool ignore_edge(int e) {
			if (pp->getEdgeVar(e).isFixed() && pp->getEdgeVar(e).isFalse()) return true;
			return false;
		}
		virtual bool ignore_node(int u) {
			if (pp->getNodeVar(u).isFixed() && pp->getNodeVar(u).isFalse()) return true;
			return false;
		}
	...
	public:
		ExplainerDijkstra(BoundedPathPropagator* _btp, int _s, std::vector<std::vector<int> > _en,
											std::vector<std::vector<int> > _in, std::vector<std::vector<int> > _ou,
											std::vector<int>& _ws)
				: FilteredDijkstra(_btp, _s, _en, _in, _ou, _ws), back(NULL), explaining(-1) {}
		ExplainerDijkstra(BoundedPathPropagator* _btp, int _s, std::vector<std::vector<int> > _en,
											std::vector<std::vector<int> > _in, std::vector<std::vector<int> > _ou,
											std::vector<std::vector<int> >& _ws)
				: FilteredDijkstra(_btp, _s, _en, _in, _ou, _ws), back(NULL), explaining(-1) {}
		...
		virtual bool ignore_edge(int e) {
			if (p->getEdgeVar(e).isFixed() && p->getEdgeVar(e).isFalse()) {
				...
				if (p->getHead(e) == p->source) return true;
				if (p->getTail(e) == p->dest) return true;
	...
	public:
		...
		virtual bool ignore_node(int n) {
			if (p->getNodeVar(n).isFixed() && p->getNodeVar(n).isFalse()) return true;
			return false;
		}
		...
		virtual bool mandatory_node(int u) {
			if (p->getNodeVar(u).isFixed() && p->getNodeVar(u).isTrue()) return true;
			return false;
		}
		virtual bool mandatory_edge(int e) {
			if (p->getEdgeVar(e).isFixed() && p->getEdgeVar(e).isTrue()) return true;
			return false;
		}
		virtual std::vector<int>& mandatory_nodes() { return p->in_nodes_list; }
...
protected:
	...
	bool _check_expl(std::vector<int> forbidden, int limit, int at, bool reverse = false) {
		...
		while (!q.empty() && count < nbNodes()) {
			...
			for (unsigned int i = 0; i < in_or_out.size(); i++) {
				...
				if (cost[other] == -1 || cost[other] > cost[curr] + ws[e]) {
					...
					if (reverse && other == source) continue;
					if (!reverse && other == dest) continue;
					q.push(new_node);
				}
			}
		}
		...
		std::cout<<std::endl;
		//*/
		if (cost[at] == -1) return false;  // assert(cost[at] != -1);
