...
public:
	...
	inline bool isFalse() const { return sat.value(body_lit) == l_False; }
};
	...
	*/
	...
	void init() {
		...
		for (int i = 0; i < raw_heads.size(); i++) {
			...
			if (so.well_founded) {
				raw_bl[i].attach(this, rules.size(), EVENT_U);
			}
		...
		for (int i = 0; i < lits.size(); i++) {
			...
			if (COMPLETE) {
				array_bool_or(b, BoolView(lits[i]));
			}
		}
