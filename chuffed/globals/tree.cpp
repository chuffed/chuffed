    ...
    if (bridgeExpl.size() > 0) {
        ...
        for (it = bridgeExpl.begin(); it != bridgeExpl.end(); it++){
            ...
            if (so.lazy) {
                ...
                if (TREEPROP_DEBUG) {
                    cout << "BRIDGE "<<bridge<<" caused by "<< cause1 <<" and "<<cause2<<" and edges ";
                    for (unsigned int i =0; i < reasons.size(); i++){
                        cout <<reasons[i]<<" ";
                    }
                    cout <<endl;
...
void TreePropagator::_findAndBuildBridges(int u, int& count, std::stack<edge_id>& s,
    ...
    for (int i = 0; i < adj[u].size(); i++) {
        ...
        if (!visited[v]){
            ...
            _findAndBuildBridges(v,count,s,depth,low,visited,parent,hits,
                                 semiExpl,bridgeExpl,articuExpl);
...
TreePropagator::TreePropagator(vec<BoolView>& _vs, vec<BoolView>& _es, 
                               vec< vec<edge_id> >& _adj, vec< vec<int> >& _en)
    : GraphPropagator(_vs,_es,_en),uf(_vs.size()), ruf(_vs.size()),nb_innodes(0),nb_avn(nbNodes()) {
    ...
    if(i >= nbNodes() && i < nbNodes()+nbEdges()) { 
        ...
        if(TREEPROP_DEBUG){
            cout << __FILE__ << " " <<__LINE__  
                 <<" edge "<<j<<" esvalue " << getEdgeVar(j).getVal()<<endl;
        }
        newFixedE.insert(j);
        ...
        if(TREEPROP_DEBUG){
            cout << __FILE__ << " " <<__LINE__  
                 <<" vsvalue " << getNodeVar(i).getVal()<<endl;
        }
        newFixedN.insert(i);
    }
...
void tree(vec<BoolView>& _vs, vec<BoolView>& _es, 
          vec< vec<edge_id> >& _adj, vec< vec<int> >& _en) {
...
void connected(vec<BoolView>& _vs, vec<BoolView>& _es,
          vec< vec<edge_id> >& _adj, vec< vec<int> >& _en) {
...
public:
    PrimStrategy(vec<Branching*>& _x, VarBranch vb, bool t)
        : BranchGroup(_x,vb,t) {
    ...
    DecInfo* branch() {
        ...
        if (false&&arg_min >= 0)
            cout<<"Tree "<<min_t<<" taking "
                ...
                <<TreePropagator::tree_propagators[0]->getEndnode(min_e,1) <<" "
                <<min_d<<endl;
        ...
        if (arg_min < 0) {
            if (!finished()) {
                for (int i = 0; i < x.size(); i++) {
                    if (!x[i]->finished())
                        return new DecInfo(x[i], false, 1);
                }
            }
            return NULL;
        }
