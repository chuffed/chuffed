...
void GraphPropagator::fullExpl(vector<Lit>& ps) {
	for (int i = 0; i < nbNodes(); i++)
		if (getNodeVar(i).isFixed()) ps.push_back(getNodeVar(i).getValLit());
	for (int i = 0; i < nbEdges(); i++)
		if (getEdgeVar(i).isFixed()) ps.push_back(getEdgeVar(i).getValLit());
}
...
GraphPropagator::GraphPropagator(vec<BoolView>& _vs, vec<BoolView>& _es, vec<vec<int> >& _en)
	...
	if (DEBUG)
		for (int i = 0; i < nbEdges(); i++) cout << i << " " << _en[i][0] << " " << _en[i][1] << endl;
...
bool GraphPropagator::propagate() {
	for (int i = 0; i < nbNodes(); i++) {
		if (getNodeVar(i).isFixed() && getNodeVar(i).isFalse())
			if (!coherence_outedges(i)) return false;
	}
	for (int i = 0; i < nbEdges(); i++) {
		if (getEdgeVar(i).isFixed() && getEdgeVar(i).isTrue())
			if (!coherence_innodes(i)) return false;
	}
	return true;
}
...
bool GraphPropagator::coherence_outedges(int node, vector<edge_id>& remvd_e) {
	for (int i = 0; i < adj[node].size(); i++) {
		...
		if (!es[edge].isFixed()) {
			...
			if (GRAPHPROP_DEBUG) cout << "COHERENCE (E) " << edge << endl;
...
bool GraphPropagator::coherence_innodes(int edge, vector<node_id>& added_n) {
	for (int i = 0; i < endnodes[edge].size(); i++) {
		...
		if (!vs[u].isFixed()) {
			...
			if (GRAPHPROP_DEBUG) cout << "COHERENCE (N) " << u << endl;
			vs[u].setVal2(true, r);
			added_n.push_back(u);
		}
	}
...
std::string GraphPropagator::available_to_dot() {
	...
	for (int e = 0; e < nbEdges(); e++) {
		if (!getEdgeVar(e).isFixed() || getEdgeVar(e).isTrue()) {
			...
			if (getEdgeVar(e).isFixed() && getEdgeVar(e).isTrue()) res += " [color = red] ";
