...
FilteredLT::FilteredLT(GraphPropagator* _p,
                                                int _r,
...
void FilteredLT::DFS(int r) {
    if(DEBUG) cout <<"DFS Visiting "<<r<<" "<< p->getNodeVar(r).isFixed()
...
bool DReachabilityPropagator::correctDominator(int r, std::vector<bool>& v, int avoid) {
    if (r == avoid) 
        return true;
    v[r] = true;
    if (DEBUG) cout <<"Visiting "<<r<<" "<< avoid<<" who is "
                    << (getNodeVar(r).isFixed()? 
                        (getNodeVar(r).isTrue()?"in":"out"):"unfixed")<<endl;
...
int DReachabilityPropagator::get_some_innode_not(int other_than) {
    for (unsigned int i = 0; i < in_nodes_list.size(); i++) {
        if (in_nodes_list[i] != other_than)
            return in_nodes_list[i];
    }
    return -1;
}

int DReachabilityPropagator::get_root_idx() {
    return root;
}
...
DReachabilityPropagator::DReachabilityPropagator(int _r, 
                                                 vec<BoolView>& _vs, 
                                                 vec<BoolView>& _es, 
                                                 vec< vec<edge_id> >& _in, 
                                                 vec< vec<edge_id> >& _out, 
                                                 vec< vec<int> >& _en) 
    :GraphPropagator(_vs,_es,_en), lt(NULL), root(_r),
...
bool DReachabilityPropagator::_propagateReachability(bool local) {
    ...
    if(DEBUG) cout <<"Reached " <<reached<<" in_nodes_tsize "<<in_nodes_tsize
                   <<" in_nodes.size() "<<in_nodes_list.size()<<endl; 
    ...
    if (reached < in_nodes_tsize) {
        ...
        if (so.lazy) {
            int some_node = -1;
            for (unsigned int i = 0; i < in_nodes_list.size(); i++){
                if(!lt->visited_dfs(in_nodes_list[i]))
                    some_node = in_nodes_list[i];
            }
            assert(some_node != -1);
            vec<Lit> psfail;
    ...
    for (unsigned int i = 0; i < in_nodes_list.size(); i++) {
        ...
        if (DEBUG) {
            ...
            cout << "Nodes: ";
            for (int i = 0; i < nbNodes(); i++)
                cout << i<<"("<<(getNodeVar(i).isFixed()? 
                                 (getNodeVar(i).isTrue()?"I":"O"):"U")<<"),";
            cout <<endl;
            cout << "Edges: ";
            for (int i = 0; i < nbEdges(); i++)
                cout << i<<"["<<getTail(i)<<","<<getHead(i)<<"]"
                     <<"("<<(getEdgeVar(i).isFixed()? 
                             (getEdgeVar(i).isTrue()?"I":"O"):"U")<<"),";
            cout <<endl;
        }
        ...
        if (!getNodeVar(dom_u).isFixed()) {
            ...
            if(DEBUG) cout <<"Adding innode "<<__FILE__<<__LINE__
                           << "("<<dom_u<<" dominates "<<u<<")"<<endl;
            ...
            if (in_deg == 1 && !getEdgeVar(last_incident).isFixed()) {
                ...
                if (local) {
                    if(!propagateNewEdge(last_incident))
                        return false;
                    if(DEBUG) cout <<"Adding inedge "<<last_incident<<" "
...
bool DReachabilityPropagator::propagateReachability() {
    return _propagateReachability(false);
}

void DReachabilityPropagator::reverseDFS(int u, vector<bool>& v, 
                                         int skip_n) {
...
void DReachabilityPropagator::reverseDFStoBorder(int u, vector<bool>& v, 
                                                 vector<bool>& my_side,
    ...
    for (int i = 0; i < in[u].size(); i++) {
        int ie = in[u][i];
        if (getEdgeVar(ie).isFixed() && getEdgeVar(ie).isFalse() &&
            lt->visited_dfs(getTail(ie)) && !my_side[getTail(ie)]) {
...
void DReachabilityPropagator::explain_dominator(int u, int dom, Clause** r) {
    if(DEBUG) cout <<"Explain dominators "
                   <<getNodeVar(get_root_idx()).isFixed()
                   <<getNodeVar(u).isFixed()<<endl;
    vec<Lit> ps;
    ps.push();
    ps.push(getNodeVar(get_root_idx()).getValLit());
    if(getNodeVar(u).isFixed() && getNodeVar(u).isTrue())
...
bool DReachabilityPropagator::checkFinalSatisfied() {
    ...
    for (int i = 0; i < nbNodes(); i++){
        if(getNodeVar(i).isFixed() && getNodeVar(i).isTrue()) {
            assert(v[i]);
            if(!v[i]) {
                cerr<< "DreachabilityPropagator not satisfied (cannot reach node "
                    <<i<<") "<<__FILE__<<":"<<__LINE__<<endl;
                return false;
            }
        }
    }
...
void dconnected(int r, vec<BoolView>& _vs, vec<BoolView>& _es,
