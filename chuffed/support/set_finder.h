...
public:
	...
	void add(std::bitset<N> key, int val) { _add(key, val, &root, 0); }

	int find(std::bitset<N> key, bool* ok) { return _find(key, ok, &root, 0); }

	void subsets(std::bitset<N> key, std::vector<std::bitset<N> >& ss, std::vector<int>& vals,
							 bool remove = false, int threshold = -1) {
		_subsets(key, ss, vals, &root, 0, std::bitset<N>(), remove, threshold);
	}

	void supersets(std::bitset<N> key, std::vector<std::bitset<N> >& ss, std::vector<int>& vals,
								 bool remove = false, int threshold = -1) {
