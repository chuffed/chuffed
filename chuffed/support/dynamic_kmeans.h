...
private:
    class ImplementedFloydWarshall : public FloydWarshall<T> {
        DynamicKMeans* _km;
    public:
        ImplementedFloydWarshall(int n, int e, DynamicKMeans* km)
            : FloydWarshall<T>(n,e), _km(km) { }
...
public:
    ...
    DynamicKMeans(int k, int _n, int _e) 
        : ClusteringAlgorithm<T>(k), n(_n), e(_e), fw(NULL) {
        fw = new ImplementedFloydWarshall(n,e,this);
    }

    virtual ~DynamicKMeans() {
        delete fw;
    }
    ...
    std::vector<int> cluster(std::vector<int> to_cluster) {
        ...
        for (unsigned int i = 0; i < to_cluster.size(); i++) {
            ...
            for (unsigned int j = 0; j < this->clusters_count; j++) {
                ...
                if (inf && !inf2) d = d2;
                else if (!inf && inf2) d = d1;
                else if (!inf && !inf2) d = MIN(d1,d2);
                else if (inf && inf2) continue; //Go to another cluster
                if (!inf || !inf2) { 
                    if (min == -1 || min > d) {
                        min = d;
                        arg_min = j;
